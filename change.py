...
HEADERS = {"Authorization": f"add your own key"}
...
def main():
    ...
    for idx, (question, student_answer) in enumerate(question_answer_pairs, start=1):
        ...
        graded_questions.append((idx, question, student_answer, obtained_marks))
    ...
    if TOTAL_MARKS == 20:
        ...
        if "2" in question_groups:
            question_groups["2"].sort(key=lambda x: x[2], reverse=True)
            filtered_questions.extend(question_groups["2"][:1])

        if "3" in question_groups:
            question_groups["3"].sort(key=lambda x: x[2], reverse=True)
            filtered_questions.extend(question_groups["3"][:1])
    ...
    if TOTAL_MARKS == 80:
        ...
        main_question_scores = []
        for main_q in ["2", "3", "4", "5", "6"]:
            if main_q in question_groups:
                total_score = sum(marks for _, _, marks, _ in question_groups[main_q])
                main_question_scores.append((main_q, total_score))

        main_question_scores.sort(key=lambda x: x[1], reverse=True)
        selected_main_questions = [q for q, _ in main_question_scores[:3]]
    ...
    # Ensure the scorecard is written properly
    with open(SCORECARD_FILE, "w", encoding="utf-8") as f:
        f.write("Question Number\tQuestion\tStudent Answer\tMarks Obtained\n")
        for idx, q, ans, marks in graded_questions:
